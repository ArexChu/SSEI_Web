...
public abstract class DaoSupportImpl<T> implements DaoSupport<T> {
	...
	// 多条件联合查询，不分页
		public List<T> getListInPageBean( QueryHelper queryHelper) {
		 ...
		 * 通过复选框的勾选项生成新的复选框字符串
		 * @param oldString
		 * @param m_Original_Data
		 * @return
		 */
		protected String genCheckBoxString(String b, String a) {
			String[] arr_a;
			if (a==null) {
				arr_a=new String[]{"0"};
			}
			...
			for (int i = 0; i < arr_b.length; i++) {
				...
				for (int j = 0; j < arr_a.length; j++) {
					if(arr_b[i].trim().equals(arr_a[j].trim())){
						k=true ;
					}
