...
<html>
    ...
    <div class="alert alert-info">当前位置<b class="tip"></b>容器管理<b class="tip"></b>检验列表</div>
    
    <span class="errorClass" id="msg">${msg}</span>
 	
	<s:form autocomplete="off" action="vesselMaintain_list">
        ...
        <tbody class="myfocus">
			  <tr>
                ...
                <td >检验状态</td>
                <td class="detail">
                	<s:select cssClass="inputClass" name="v_finished" list="#{'1':'检验中','2':'已完成'}" headerKey="" headerValue="请选择检验状态"  />
      ...
      <table class="table table-striped table-bordered table-condensed center" id="list">
        	...
        	<tbody>
				...
				<tr>
					...
					<s:if test="%{v_finished==1}">
 							<s:a action="vesselMaintain_updateStatus?id=%{id}" onclick="return confirm('您确定已完成检验吗？')" >
					...
					<td>
						<s:a  action="%{v_M_Mode == '定期' ? 'vesselPeriodicalMaintain' : v_M_Mode=='年度' ? 'vesselAnnualMaintain' : 'vesselCheckMaintain'}_list?vesselMaintainId=%{id}">
						<div class="btn btn-mini btn-primary reverse">
						检验项目
						</div>
						</s:a>
						<s:a  action="%{v_M_Mode == '定期' ? 'vesselPeriodicalMaintain' : v_M_Mode=='年度' ? 'vesselAnnualMaintain' : 'vesselCheckMaintain'}_toFileList?vesselMaintainId=%{id}">
						<div class="btn btn-mini btn-primary reverse">
						附件
						</div>
						</s:a>						
 						<s:a action="vesselMaintain_delete?id=%{id}" onclick="return confirm('您确定要删除吗？')" >
 						<div class="info2 btn btn-mini btn-danger">
						 删除
						</div>
						</s:a>
