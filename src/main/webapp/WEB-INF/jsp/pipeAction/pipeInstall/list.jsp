...
<html>
	...
	<s:form autocomplete="off" action="pipeInstall_list">
        ...
        <tbody class="myfocus">
             ...
             <tr>
             	 <td>检验状态</td> 
             	<td class="td_detail">
                    <s:select name="pi_finished" list="#{'1':'检验中','2':'已完成'}" headerKey="" headerValue="检验状态"/>
				</td>
      ...
      <table class="table table-striped table-bordered table-condensed center" id="list">
        	...
        	<tbody>
				...
				<tr>
					...
					<td>${pi_Check_Date}</td>
					<td>
						<s:if test="%{pi_finished==1}">
	 							<s:a action="pipeInstall_updateStatus?id=%{id}" onclick="return confirm('您确定已完成检验吗？')" >
					...
					<td>
						<s:a  action="pipeInstall_editUI?id=%{id}">
						<div class="btn btn-mini btn-primary reverse">
						查看
						</div>
						</s:a>
						<s:a  action="pipeInstall_toMenuUI?id=%{id}">
						<div class="btn btn-mini btn-primary reverse">
						安装项目
						</div>
						</s:a>
						<s:a  action="pipeInstall_toFileList?id=%{id}">
						<div class="btn btn-mini btn-primary reverse">
						附件
						</div>
						</s:a>					
 						<s:a action="pipeInstall_delete?id=%{id}" onclick="return confirm('您确定要删除吗？')" >
 						<div class="info2 btn btn-mini btn-danger">
						 删除
						</div>
						</s:a>						
